 ...
 * wrong.
 */
export function html({ size, brand, headline, sub, eyebrow, cta, fine, mark, bg }) {
  const s = SIZES[size];
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .frame{position:relative;width:100%;height:100%;padding:${s.pad}px;display:flex;
    ${banner
  ...
  .head{white-space:${banner ? "nowrap" : "normal"};font-family:'${f.displayFamily}',serif;font-weight:${f.displayWeight};
    font-size:${s.head}px;line-height:${f.displayLineHeight};letter-spacing:${f.displayTracking};color:${c.ink};
    margin:${banner ? 0 : Math.round(s.head * 0.16)}px 0}
  .rule{width:${s.rule}px;height:${Math.max(2, Math.round(s.w / 360))}px;background:${c.accent};margin-bottom:${Math.round(s.body * 0.9)}px}
